...
public class tisHermitBoosterMod implements
    ...
    public static void initialize() {
        new tisHermitBoosterMod();
    }

    public tisHermitBoosterMod() {
    ...
    private static String checkResourcesPath() {
        String name = tisHermitBoosterMod.class.getName(); //getPackage can be iffy with patching, so class name is used instead.
        ...
        if (!resources.exists()) {
            throw new RuntimeException("\n\tFailed to find resources folder; expected it to be at  \"resources/" + name + "\"." +
                    ...
                    "\t\"private static final String resourcesFolder = checkResourcesPath();\"\n" +
                    "\tat the top of the " + tisHermitBoosterMod.class.getSimpleName() + " java file.");
        }
    ...
    private static void loadModInfo() {
        Optional<ModInfo> infos = Arrays.stream(Loader.MODINFOS).filter((modInfo)->{
            ...
            return initializers.contains(tisHermitBoosterMod.class.getName());
        }).findFirst();
        if (infos.isPresent()) {
            info = infos.get();
            modID = info.ID;
